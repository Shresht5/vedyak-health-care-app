...
import { addReport, deleteReport, getReport, updateReport } from '../services/db/Reports'

const AddReports = ({ route, navigation }: any) => {
    ...
    useEffect(() => {
        const load = async () => {
            ...
            setDocument({
                ...data,
                images: (data.images || []).map((uri: string) => ({
                    uri,
                })),
                pdf: (data.pdf || []).map((uri: string) => ({
                    uri,
                })),
            });
        };
